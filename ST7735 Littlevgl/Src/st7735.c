...
void ST7735_DrawImage_pixel(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t* data) {
    if((x >= ST7735_WIDTH) || (y >= ST7735_HEIGHT))
        return;
		if((x + w - 1) >= ST7735_WIDTH) return;
    if((y + h - 1) >= ST7735_HEIGHT) return;
    ...
    ST7735_Select();
		...
		for(y=0; y<h; y++)
		{
			...
			{
				ST7735_SetAddressWindow(x, y, x+1, y+1);
				uint8_t _data[] = { data[counter] >> 8, data[counter] & 0xFF };
				ST7735_WriteData(_data, sizeof(_data));
				counter++;
			}
		}
...
void ST7735_line_h(uint8_t x0, uint8_t x1, uint8_t y0, uint8_t width, uint16_t color)
{
	if(x0>x1)
	{
		uint8_t temp = x0;
	...
	if(width%2)
	{
		...
		for(i=0;i<width;i++)
			for(j=x0;j<=x1;j++)
				ST7735_DrawPixel(j, start+i, color);
	}
	else
	{
...
void ST7735_draw_rectangle(uint8_t x1, uint8_t x2, uint8_t y1, uint8_t y2, uint16_t color)
{
	uint8_t i,j,buffer;
	if(x1>x2)
	{
