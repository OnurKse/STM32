...
void pushBlock(uint16_t color, uint32_t len)
{
  ...
  if  (len<BUF_SIZE) {
    ...
    HAL_SPI_Transmit(&hspi2, (uint8_t*)col, len<<1, HAL_MAX_DELAY);
    return;
  }

  for (uint32_t i = 0; i < BUF_SIZE; i++) col[i] = swapColor;
  do {
    HAL_SPI_Transmit(&hspi2, (uint8_t*)col, BUF_SIZE<<1, HAL_MAX_DELAY);
    len -= BUF_SIZE;
  } while ( len>=BUF_SIZE ) ;
  // Send remaining pixels
  if (len) HAL_SPI_Transmit(&hspi2, (uint8_t*)col, len<<1, HAL_MAX_DELAY); //*/
}
